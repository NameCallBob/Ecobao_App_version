...
function GoodInfo({ modalVisible, setModalVisible, data}) {
    if(!data){return null}
    const [Goods_detail,setGD] = useState(null)
    useEffect(() => {
        Axios().get("Goods/id/",{params:{gid:data}})
            .then((res) =>{
                const resp = res.data
                setGD(resp)
    ...
    >
        {Goods_detail && Goods_detail.map((Goods) => (

          <TouchableWithoutFeedback onPress={() => setModalVisible(false)}>
            <View style={styles.centeredView}>
                <View style={styles.modalView}>
                    <Text style={styles.modalTitle}>{Goods.name}</Text>
                    <Text style={styles.modalType}>NT${Goods.price}</Text>
                    <QuantityButton gid = {Goods.gid} modalVisible={modalVisible} setModalVisible={setModalVisible}/>
                </View>  
            </View>
